...
export async function loadLeadDashboard(pool, requestedLimit = DEFAULT_LIMIT) {
  ...
  const [summaryResult, leadsResult] = await Promise.all([
    ...
    pool.query(`
      ...
      GROUP BY lead.id, latest.event_type, latest.occurred_at
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font:15px/1.5 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
    ...
    .metrics{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:12px;margin-bottom:18px}.metric{background:#fff;border:1px solid var(--line);border-radius:16px;padding:18px}.metric strong{display:block;font:600 28px Georgia,serif}.metric span{color:var(--muted);font-size:12px}
...
<main>
  ...
  <div class="table-wrap"><table><thead><tr><th>Letzte Aktivität</th><th>Status</th><th>Intent</th><th>Interesse</th><th>Letzter Schritt</th><th>Kontakt</th><th>Session</th></tr></thead><tbody id="rows"></tbody></table><div class="empty" id="empty" hidden>Keine Leads für diesen Filter.</div></div>
</main>
<script>
  const state={leads:[]};
